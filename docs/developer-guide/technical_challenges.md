...
migration owns a version step) makes the migrations idempotent — having replaced
the old per-feature `playbook_migrated` / `hooks_migrated` latches (since removed
now that all configs carry an explicit `schema_version`).
