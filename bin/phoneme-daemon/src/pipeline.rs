...
// The per-step runners + on-demand entry points moved into the submodules above.
// Re-export the ones referenced from here (the recipe executor calls the runners
// unqualified) or from other daemon modules (`pipeline::extract_entities`, the
