...
pub async fn run(args: TagArgs, cfg: &Config, is_json: bool) -> ExitCode {
    ...
    match args.action {
        ...
        TagAction::Add { name, color } => {
            match conn.send(Request::AddTag { name, color }).await {
                Ok(val) => {
                    if is_json {
                        println!("{}", serde_json::to_string_pretty(&val).unwrap());
                    } else {
        ...
        TagAction::Delete { id } => {
            match conn.send(Request::DeleteTag { id }).await {
                ...
                match conn.send(Request::ListTags).await {
                    Ok(val) => {
                        ...
                        if let Some(t) = tags.into_iter().find(|t| t.name == tag) {
                            match conn.send(Request::AttachTag { recording_id: id, tag_id: t.id }).await {
                                Ok(_) => ExitCode::SUCCESS,
                                Err(e) => e,
                            }
                        } else {
            ...
            if let Some(id) = phoneme_core::id::RecordingId::parse(&recording_id) {
                match conn.send(Request::ListTags).await {
                    Ok(val) => {
                        let tags: Vec<phoneme_core::tags::Tag> = serde_json::from_value(val).unwrap_or_default();
                        if let Some(t) = tags.into_iter().find(|t| t.name == tag) {
                            match conn.send(Request::DetachTag { recording_id: id, tag_id: t.id }).await {
                                Ok(_) => ExitCode::SUCCESS,
                                Err(e) => e,
                            }
                        } else {
