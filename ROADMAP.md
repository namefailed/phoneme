...
- [ ] **Doctor: rebuild catalog (GUI)** — ⚠️ **needs a product decision; do NOT just wire the CLI to a button.** Investigation (2026-06-15) found the CLI `doctor --rebuild-catalog` is **destructive, not a rebuild**: it deletes `catalog.db` (+ WAL), and nothing re-imports — transcripts/titles/tags/notes/summaries live **only** in `catalog.db` (no file sidecars; `reconcile.rs` only requeues inbox orphans + sweeps stale rows; there is no audio-dir → catalog scan). So a fresh start yields an **empty** library, not a rebuilt one — the CLI's "rebuilds from the inbox + audio files" comment is inaccurate. **Recommended GUI feature instead:** a *non-destructive* "Re-import recordings from disk" — walk the audio dir, find supported audio files with no catalog row (compare canonical `audio_path`), and import each (insert a `Queued` row referencing the existing file + enqueue for transcription, reusing the `ImportRecording` machinery), never deleting existing rows. That genuinely recovers a lost/partial catalog and matches the CLI's *claimed* intent, safely. Deferred pending that build (the destructive delete should not reach the GUI; the CLI wording should also be corrected).
