...
- ✅ **In-recording speaker correction — detail-pane UI** *(shipped — see CHANGELOG)* — the interactive correction UI in the recording detail/merged view (click a turn → reassign; select turns → split; pick two speakers → merge) now rides the backend, wire contract, and CLI that already shipped (`catalog::{reassign_segment,merge_speakers,split_speaker}`, the matching IPC + Tauri commands, `phoneme speaker reassign|merge|split`; segments stay authoritative and the prose `[Speaker N]:` markers rebuild in-transaction).

### Foundation & tech-debt
- ✅ **Kill the hook double-fire hazard** *(shipped — see CHANGELOG)* — the legacy `hook.commands` loop in `pipeline::run` is now gated behind `schema_version < CURRENT_SCHEMA_VERSION`, so a stale/hand-edited config (a migrated schema version still carrying non-empty `hook.commands`) can no longer fire shell hooks twice alongside the recipe-driven `run_hook_steps`. A single-fire regression test pins the invariant.
- ✅ **CI hardening** *(shipped — see CHANGELOG)* — the `cargo audit` / `pnpm audit` jobs are blocking now (no `continue-on-error`), and the workspace test job dropped `--test-threads=1` to run in parallel (per-test DB isolation — in-memory + per-test tempdir — means parallel CI catches isolation rot instead of masking it).
- ✅ **Opt-in, local-only diagnostics bundle** *(shipped — see CHANGELOG)* — a Doctor **Export diagnostics** button writes a sanitized JSON snapshot (app/OS info, masked config, bounded daemon-log tail) under `<data_dir>/diagnostics/` for bug reports. No audio/transcripts/network/plaintext secrets; fully consistent with the no-telemetry privacy posture.
