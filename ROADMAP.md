...
- [x] **Doctor: rebuild catalog (GUI)** — shipped on BOTH paths. **(1) Non-destructive (the original recommendation):** "Re-import from disk" in Settings → Doctor (`reimport_from_disk`) walks the audio dir, finds supported files with no catalog row, and imports each (`Queued`, reusing the import machinery) — never deleting rows. This is the safe everyday recovery. **(2) Destructive rebuild (shipped intentionally, 2026-06-17):** a Doctor "Rebuild catalog from disk" button + in-process `RebuildCatalog` IPC that clears every row then re-imports every WAV (so the library re-populates + re-transcribes rather than going empty like the old CLI delete). It's a deliberate, guarded power-tool: refused while a recording/meeting is active, behind an arm-to-confirm in the UI, and it loses transcripts/edits/tags (those live only in `catalog.db`) by design — for genuine `catalog.db` *corruption* (daemon can't open it) the CLI remains the tool. *(Prior note kept for history: the CLI `doctor --rebuild-catalog` is a destructive delete with no re-import — its "rebuilds from the inbox + audio files" comment is still inaccurate and should be corrected.)*
...
- [ ] **Diarization quality** *(prereq for named speakers — don't build naming UX on wrong labels)*. Each item below was verified against `diarization.rs` / `transcription.rs` and the `speakrs 0.4.2` source; verdicts noted inline.
  ...
  - [x] **Track-aware Meeting Mode** ✓ *(shipped)*. The pipeline now reads a recording's `track` + `meeting_id` (a narrow two-column `Catalog::track_and_meeting` read) BEFORE building the provider, and threads a `DiarizationTrack` hint into `transcribe_with_segments` — `Diarize` (default) for normal recordings and the meeting system/loopback track, `FixedSpeaker("You")` ONLY when `meeting_id.is_some()` AND `track == "mic"`. The local OpenAI-compatible path acts on the hint: a `FixedSpeaker` mic track skips speakrs entirely (it never loads or runs, even with local diarization configured) and wraps the whisper segments under one `[Speaker 1]` turn via `diarization::label_all_as`. The daemon then writes a `speaker_names` row mapping label 1 → "You", so the UI renders "You" while the transcript keeps its canonical `[Speaker N]` markers (the `diarized` detection and merged-meeting view are unchanged) and the name stays user-renamable. Cloud/ElevenLabs providers ignore the hint (a meeting mic track on a cloud STT backend is an edge case). Net effect: a meeting triggers one diarization load+run (system track) instead of two, and a single-voice mic track can no longer be split into spurious speakers. *(transcription.rs `DiarizationTrack` + `label_all_as`; pipeline.rs row fetch + gating + "You" row; 6 new unit/integration tests incl. a mic-track-labelled-without-diarizing daemon test and a system-track negative case.)*
  - [x] **Word-level alignment instead of 1 s segments** ✓ *(shipped)*. Was: whisper **segments** × diarization turns, with `assign_speakers` attributing each whole segment by max overlap. Now the local OpenAI-compatible path attributes each *word* to a speaker off the per-frame activation matrix: `run_local_diarization` returns a `LocalDiarization` carrying `discrete_diarization` (`Array2<f32>` frames×speakers, made exclusive so each frame has one winner), and `diarization::assign_words` maps each word's `[start,end]` to its frame range `[⌊start/STEP⌋,⌊end/STEP⌋]`, sums activations per speaker column, and argmaxes — column `k` → label `SPEAKER_{k:02}` → the SAME stable 1-based `[Speaker N]` index `label_segments` uses, so word- and segment-level numbering agree. Consecutive same-speaker words group into turns for the formatted text, the persisted `TranscriptSegment` timeline, and the per-word speaker tags (all built in one pass). When word timestamps are absent (cloud STT, segments-only, empty words) it falls back to the segment-level path, so a one-word-per-segment transcript reproduces the old labels exactly. The `speaker-count ≤ 1 → plain text` gate is preserved. `LocalDiarization` also surfaces `embeddings` / `hard_clusters` / `segmentations` verbatim for the deferred named-speaker-voiceprint work, so its return type isn't rewritten twice. *(diarization.rs `assign_words` + `LocalDiarization`; transcription.rs `diarize_transcript` → `diarize_per_word` / `diarize_per_segment`; built on the v1.9 word-timestamp substrate. 10 new unit tests incl. a boundary-straddling word and the fallback-equivalence regression.)*
  - [x] **Turn-smoothing scope — decided, no change** ✓ *(closed by analysis)*. The
    word-level path already carries the heavy island/flicker smoothing
    (`smooth_word_speaker_runs` + `backfill_unattributed_words` +
    `coalesce_subword_tokens`, the v1.8 diarization rework). Extending it to the
    other two paths was evaluated and **deliberately not done**: the segment path
    (`label_segments`) is fed `SpeakerSpan`s that are already centroid-merged +
    gap-smoothed (`SPEAKER_MERGE_GAP_SECS`) and is coarse — a whole sentence rarely
    flips — and forcing the per-word smoothing there would risk *merging genuine
    short interjections* ("Yes." between another speaker's sentences); the cloud
    paths (Deepgram/AssemblyAI) are diarized server-side with commercial-grade
    smoothing, so layering the local-noise-tuned heuristic on top would fight a
    better model. Net: the smoothing stays where the per-frame-argmax noise it
    targets actually lives (the local word path). Revisit only if a real cloud/
    segment-path flicker case turns up.
