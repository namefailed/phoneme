...
describe("formatTime", () => {
  it("returns a non-empty string for both modes", () => {
    expect(formatTime("2026-01-15T15:00:00.000Z", true)).toBeTruthy();
    expect(formatTime("2026-01-15T15:00:00.000Z", false)).toBeTruthy();
  });

  it("produces different output for 12h vs 24h mode on the same timestamp", () => {
    const twelveHour = formatTime("2026-01-15T03:00:00.000Z", false);
    const twentyFourHour = formatTime("2026-01-15T03:00:00.000Z", true);
    expect(twelveHour).not.toBe(twentyFourHour);
  });

  it("24h output does not contain AM or PM markers", () => {
    const result = formatTime("2026-01-15T15:00:00.000Z", true);
