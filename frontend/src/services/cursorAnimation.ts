 ...
 *
 * It's purely additive: the real `.kbd-cursor` outline still marks the
...
function place(el: HTMLElement, animate: boolean) {
  ...
  ensureEls();
  const g = ghost!;
  // Decide glide-vs-snap before reassigning `current` (we want to compare against
  // the old target). The glow glides only for row-to-row moves within one pane. It
  // snaps instantly when the cursor crosses into a different pane, or when the ghost
  // is currently hidden — otherwise it visibly flies across the whole window from
  // wherever it last sat. That stale spot is usually a list row: the first time a
  // detail pane opens its editors haven't laid out yet, so the transcript cell's
  // place() gets clipped/skipped and `current` stays on the list — then the next
  // detail cell (notes, say) would glide all the way from the list. Snapping
  ...
  // Feather the right edge when the rect got clipped to its pane (a full-width list
  // row clipped at the detail-pane boundary): the cut dissolves instead of showing a
  // hard line, so it reads as tucked behind the pane rather than as a box that stops
  // dead at — or spills over — the edge.
  const mask =
  ...
  // The smear/trail "streak" is deliberately a no-op. A streak that drew a
  // translucent box bounding the old and new rects (min/max of both) and faded it
  // out looks like a flash: on a move where that bounding box is sizeable it shows
  // for a frame as a box around the whole move before fading, worst in the
  // mid-distance range (far moves get skipped by a tall-union guard, so they look
  // clean). That box was never a real directional smear anyway, so every mode just
  // glides. A proper motion-trail smear — a tapered shape along the path rather
  // than a box — could be built separately if we want one.
  if (tail) tail.style.opacity = "0";

  // Glide and resize together (mini.animate-style): position and size share one
...
/** Settle on the element the roving cursor most recently landed on (or nothing).
 *  There can be several `.kbd-cursor` nodes at once — the list keeps a dimmed one —
 *  so the ghost follows whichever was just activated, picked out of the mutation
 *  batch. */
function flush() {
  raf = 0;
...
/** Did `el` just gain class `cls` in this mutation (compared to its old className)? */
...
function onMutations(records: MutationRecord[]) {
  ...
  for (const rec of records) {
    ...
    // A pane just took focus (sidebar / list / detail). This covers the case where
    // the inner highlight class didn't change — returning to the list with the same
    // row focused, say — so the ghost still moves to that pane's live cursor
...
/** Focus entered a text-editing surface, so hide the glow — it shouldn't sit over
 *  the text you're typing (the transcript/notes CodeMirror, a tag-name input). */
 ...
 *  back onto the current cursor (Shift+Esc out of the editor, or Esc out of the
 *  tag input). */
...
/** A mouse click switches to mouse mode: hide the glow now and keep it hidden as
 *  the panes' focus-follow moves the roving cursor underneath (place() still tracks
 *  `current`, it just stays invisible). Capture phase so this lands before the pane
 *  click-handlers update the cursor. */
...
/** A real keypress switches to keyboard mode: the user is taking over from the
 *  keyboard, so reveal the glow at the current cursor (the spot a prior mouse click
 *  left it). Bare modifier presses don't count. Capture phase so the flag is set
...
function onKeyInput(e: KeyboardEvent) {
  ...
  // resulting place() glides on from here a moment later. Stays hidden while the
...
function connect() {
  ...
  // pane. This is scoped to list cursors and done instantly (see onPaneResize): a
  // blanket observer that also re-placed detail cursors makes leaving the list
  // "grow then jump", so it stays list-only.
  ensureListObserved();
  ...
  // phase settles the flag before the pane/keyboard handlers run.
...
/** The list pane resized — re-clamp a list cursor instantly so the glow tracks the
 *  (moved) detail-pane edge instead of spilling over it. No animation, and only for
 *  list cursors: re-clamping a detail/sidebar cursor as its own pane animates open
 *  is what makes leaving the list "grow then jump". rAF-coalesced so a splitter drag
 *  or an enter animation doesn't thrash. */
 ...
 *  Meeting"/"Stop"), Play ↔ Pause — the glow re-fits the new box instead of keeping
 *  the old one's frame. Re-targets whenever `current` changes; a no-op when it's
 *  already the node we watch. The re-fit is instant (no glide) so the glow hugs the
 *  control as it resizes rather than lagging a frame behind its own edge. */
...
/** Seed the glow onto a specific control right now, gliding in from wherever it
 *  sits. The MutationObserver only catches class *changes* on existing nodes, so a
 *  control highlighted by a fresh render — a popover that opens with its active
 *  option already `.kbd-cursor` — is missed, and the glow wouldn't follow until the
 *  next move. Components call this on open so the glow lands with the highlight.
 *  No-op when the animation is off or the element isn't live. */
