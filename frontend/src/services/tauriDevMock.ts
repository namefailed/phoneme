...
let config: Record<string, unknown> = {
  ...
  playbook: [
    { id: "cleanup", name: "Cleanup", description: "Tidy stutters, repetitions, and phonetic slips while keeping the original tone.", builtin: true, kind: "transform", target: "",
      llm: { provider: "", model: "", prompt: "Clean up any stuttering, repetitions, or phonetic inaccuracies from the transcript. Maintain original tone.", api_url: "", timeout_secs: 30 }, hook: { command: "", webhook_url: "", timeout_secs: 60 } },
    { id: "title", name: "Title", description: "Generate a short title for the recording.", builtin: true, kind: "enrichment", target: "title",
      llm: { provider: "", model: "", prompt: "You title voice-note transcripts. Reply with ONLY a short title for the transcript: at most 8 words, plain text, no quotes, no trailing punctuation, no preamble.", api_url: "", timeout_secs: 30 }, hook: { command: "", webhook_url: "", timeout_secs: 60 } },
    { id: "summary", name: "Summary", description: "Summarize the transcript into a few clear bullet points.", builtin: true, kind: "enrichment", target: "summary",
      llm: { provider: "", model: "", prompt: "Summarize the following transcript concisely as a few clear bullet points capturing the key topics, decisions, and any action items. Output only the summary, with no preamble.", api_url: "", timeout_secs: 30 }, hook: { command: "", webhook_url: "", timeout_secs: 60 } },
    { id: "auto_tag", name: "Auto-tag", description: "Suggest tags for the recording (you approve before they apply).", builtin: true, kind: "enrichment", target: "tags",
      llm: { provider: "", model: "", prompt: "Suggest a few short topical tags for this transcript. Reply with ONLY a comma-separated list of lowercase tags, no preamble.", api_url: "", timeout_secs: 30 }, hook: { command: "", webhook_url: "", timeout_secs: 60 } },
    { id: "prompt_polish", name: "Prompt polish", description: "Reshape the transcript into a clean, well-structured LLM prompt.", builtin: false, kind: "transform", target: "",
      llm: { provider: "ollama", model: "", prompt: "Rewrite the following dictation into a single clear, well-structured prompt for an AI assistant. Keep the intent; fix grammar; output only the prompt.", api_url: "", timeout_secs: 45 }, hook: { command: "", webhook_url: "", timeout_secs: 60 } },
    { id: "action_items", name: "Action items", description: "Extract action items into a custom field.", builtin: false, kind: "enrichment", target: "custom:action_items",
      llm: { provider: "", model: "", prompt: "List any action items from this transcript as a short bulleted list. If there are none, reply 'None'.", api_url: "", timeout_secs: 30 }, hook: { command: "", webhook_url: "", timeout_secs: 60 } },
    { id: "journal", name: "Append to journal", description: "Append the transcript to a daily journal file.", builtin: false, kind: "hook", target: "",
      llm: { provider: "", model: "", prompt: "", api_url: "", timeout_secs: 30 }, hook: { command: 'powershell -Command "$d=($input|Out-String|ConvertFrom-Json); Add-Content $HOME\\\\journal.md $d.transcript"', webhook_url: "", timeout_secs: 60 } },
  ],
  recipes: [
