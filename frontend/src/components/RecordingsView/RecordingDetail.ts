...
export class RecordingDetail {
  ...
  private renderRecording() {
    ...
    // Notes editor using CodeMirror (Vim mode)
    const notesEditorRoot = this.container.querySelector<HTMLElement>("#notes-editor");
    if (notesEditorRoot) {
      this.notesEditor?.dispose();
      this.notesEditor = new NotesEditor(notesEditorRoot, r.id, r.notes ?? "", (d) => {
        this.dirty = d || (this.editor?.getText() !== r.transcript);
      });
    }
  }
  ...
  saveDirtyEdits(): Promise<void> {
    const promises = [];
    if (this.editor) promises.push(this.editor.save());
    if (this.notesEditor) promises.push(this.notesEditor.save());
