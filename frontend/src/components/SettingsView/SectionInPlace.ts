...
import { prettyModel } from "./inPlaceModels";
import {
  DICTATION_HISTORY_KEEP,
  recipeOptions,
  renderVoiceCommands,
  renderSnippets,
  renderAppOverrides,
  renderAppRecipes,
  renderContextDenylist,
...
export class SectionInPlace {
  ...
  private render() {
          ...
          <div style="display: flex; flex-direction: column; align-items: flex-start; gap: 8px; width: 100%;">
            ...
            <div style="display: flex; gap: 6px; width: 100%; align-items: center;">
              <input id="ip-app-recipe-add-name" type="text" placeholder="App executable (e.g. Outlook.exe)"
                style="flex: 1 1 auto; min-width: 0;" />
              <select id="ip-app-recipe-add-recipe">${recipeOptions(this.config, "")}</select>
    ...
    renderAppOverrides(this.container, this.config);
    renderAppRecipes(this.container, this.config);
    renderContextDenylist(this.container, this.config);
    renderVoiceCommands(this.container, this.config);
    renderSnippets(this.container, this.config);
    this.renderSttDetail();
    void renderDictationHistory(this.container);
  }
