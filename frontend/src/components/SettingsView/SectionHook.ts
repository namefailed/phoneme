...
export class SectionHook {
  ...
  constructor(
    ...
    // Seed the [webhook] table so the safety toggles + signing/headers can bind
    // to it (setByPath throws on a missing parent). The knobs default off — the
    // safe posture the backend ships. `hmac_secret` arrives masked (the tray
    // replaces a set secret with the keep-sentinel before it reaches us).
    ...
    // Edit headers via an ordered row list, synced back to the map on every
    // change (a plain object can't be edited row-by-row while keys are renamed).
