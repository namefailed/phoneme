...
//!
//! The `run_local_checks` function is synchronous (called from a `#[tauri::command]`).
//! The `run_backend_checks` function is async and probes remote HTTP endpoints
//! (Whisper, Ollama, OpenAI) with short timeouts so the Doctor UI stays snappy.

use phoneme_core::Config;
use serde::Serialize;

/// Result for a single Doctor check item. `fix_action` is an opaque string
/// the frontend switches on to dispatch the right remediation UI (e.g.
/// launching the daemon or opening a file).
#[derive(Debug, Clone, Serialize)]
pub struct CheckResult {
    pub name: String,
    pub ok: bool,
    pub detail: String,
    /// Opaque token the frontend uses to decide what "Fix" does.
...
/// LLM post-processing is enabled, also probes Ollama / OpenAI. Each probe
/// uses a 3-second timeout so the UI doesn't hang on unreachable services.
pub async fn run_backend_checks(cfg: &Config) -> Vec<CheckResult> {
    ...
    out.push(CheckResult {
        ...
        // Not a fix_action because Ollama is optional; user needs to install it separately.
        fix_action: None,
    });
