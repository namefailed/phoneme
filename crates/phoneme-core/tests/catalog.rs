...
async fn hybrid_search_filter_restricts_to_matching_recordings() {
    ...
    let unscoped = catalog
        .hybrid_search("", &q, 10, -1.0, None)
        .await
        .unwrap();
    assert_eq!(unscoped.len(), 2, "unscoped search returns both: {unscoped:?}");
    ...
    let scoped = catalog
        ...
        .await
        .unwrap();
    assert_eq!(scoped.len(), 1, "scoped search restricts to the tag: {scoped:?}");
...
async fn hybrid_search_filter_ignores_query_pagination_fields() {
    ...
    let rec = sample_recording(RecordingId::new());
    catalog.insert(&rec).await.unwrap();
    catalog.upsert_embedding(&rec.id, &[1.0, 0.0, 0.0]).await.unwrap();
...
async fn find_replace_replaces_and_counts_case_insensitive_by_default() {
    ...
    catalog.insert(&rec).await.unwrap();
    catalog
        .update_transcript(&rec.id, "The cat sat. THE end.", "The cat sat. THE end.", "m")
        .await
        .unwrap();
    ...
    assert!(got.user_edited, "find-replace marks the recording user-edited");
    let original = catalog.get_original_transcript(&rec.id).await.unwrap();
    assert_eq!(
        original.as_deref(),
